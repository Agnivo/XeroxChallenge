...
def getfeatures2(
    ...
    for it, id in enumerate(ids):
        ...
        featsums = [0 for i in xrange(32)]
        featcnts = [0 for i in xrange(32)]
        featmaxs = [0 for i in xrange(32)]
        featmins = [1e10 for i in xrange(32)]

        cfeat = [np.nan for i in xrange(32)]
        ...
        for i in xrange(ivitals.shape[0]):
            ...
            feat = np.hstack((ivitals[i].ravel(), ilabs[i].ravel()))
            for j in xrange(32):
                if not np.isnan(feat[j]):
                    cfeat[j] = feat[j]
                    featsums[j] += feat[j]
                    featcnts[j] += 1
                    featmaxs[j] = max(featmaxs[j], feat[j])
                    featmins[j] = min(featmins[j], feat[j])

            temp = []
            for j in xrange(32):
                if featcnts[j] > 0:
                    temp.extend([cfeat[j], featsums[j] * 1.0 / featcnts[j],
                                 featmaxs[j], featmins[j]])
                else:
                    temp.extend([feat_means[j], feat_means[j],
            ...
            if folds[it] != valfold or full:
                if target == 1 and ttarget == 0:
                    continue
                writecsvline(trainfeats, np.hstack((temp, [age])))
                writecsvline(traintargets, [id, time, ttarget, icu])
            else:
                writecsvline(valfeats, np.hstack((temp, [age])))
                writecsvline(valtargets, [id, time, ttarget, icu])
