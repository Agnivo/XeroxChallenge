...
'''
    Assuming 3. For each patient, predictions are present
    for all timestamps where ICU flag == 1 <=>
...
'''
    Assuming 4. For each patient, predictions are present
    for only those timestamps where ICU flag == 1 <=>
    Some patients have prediction = 1 when ICUFlag == 0,
    return false, else, returm true
'''
...
def validate(predfile, truefile):
    ...
    #     finalScore = 0
        ...
        medianPredictionTimeScore = (
            float(medianPredictionTimeClipped) / 72)
